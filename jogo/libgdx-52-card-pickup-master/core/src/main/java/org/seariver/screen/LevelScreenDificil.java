...
public class LevelScreenDificil extends BaseScreen {

    private ArrayList<Pile> pileList;
    private Label messageLabel;
    public static float tempodificil;

    public LevelScreenDificil(int MAXTEMPO) {
        super(MAXTEMPO);
    }
    ...
    public void initialize() {
        ...
        background.loadTexture("dificil.jpg");
    ...
    public void update(float deltaTime) {
        ...
        if (complete) {
            messageLabel.setText("Você venceu o level difícil");
            messageLabel.setVisible(true);

            tempoRestante = (MAXTEMPO - (System.currentTimeMillis() - tempoRestante) / 1000);
            tempodificil = tempoRestante;
        }
